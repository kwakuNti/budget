...
try {
    ...
    $next_step_name = $next_step ? $next_step['step_name'] : $current_step_name; // Keep current step if no next step
    $redirect_url = $next_step ? '../' . $next_step['page_url'] : null;
    $is_completed = !$next_step;
    
    // Update progress
    if ($is_completed) {
        // If walkthrough is completed, set is_completed = 1 and completed_at
        $stmt = $conn->prepare("UPDATE user_walkthrough_progress SET steps_completed = ?, is_completed = ?, completed_at = ? WHERE user_id = ? AND walkthrough_type = 'initial_setup'");
        $completed_at = date('Y-m-d H:i:s');
        $steps_json = json_encode($steps_completed);
        $is_completed_int = 1;
        $stmt->bind_param("sisi", $steps_json, $is_completed_int, $completed_at, $user_id);
    } else {
        // If not completed, update current_step to next step
        $stmt = $conn->prepare("UPDATE user_walkthrough_progress SET current_step = ?, steps_completed = ? WHERE user_id = ? AND walkthrough_type = 'initial_setup'");
        $steps_json = json_encode($steps_completed);
        $stmt->bind_param("ssi", $next_step_name, $steps_json, $user_id);
    }
